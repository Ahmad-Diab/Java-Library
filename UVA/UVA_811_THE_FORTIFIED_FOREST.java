...
public class UVA_811_THE_FORTIFIED_FOREST {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			double length = 0;
			int currTrees = Integer.MAX_VALUE ;
			...
			for (int i = 1; i < (1 << (n)) - 1; i++) {
				...
				for (int j = 0; j < 31; j++) {
					if ((i & 1 << j) != 0) {
						...
						a[size++] = new Point (p[j].x , p[j].y);
						
					
				}
				
				double perimeter = (Polygon.convexHull(Arrays.copyOf(a, size))).perimeter();
				
				if(costSubset < perimeter || min < valSubset || (min == valSubset && currTrees <= trees)) continue;
				
				
				curr = Arrays.copyOf(ans, currSize);
				
				min = valSubset ;
				
				
					
				length =  costSubset - perimeter ;
				

			}

			out.printf("Forest %d\n", cases++);
			out.printf("Cut these trees:");
			for (int x : curr)
				out.print(" " + x);
			out.print("\n");
			out.printf("Extra wood: %.2f\n", length );
	...
	static class Polygon {
		...
		static Polygon convexHull(Point[] ptt) // all points are unique, remove duplicates, edit ccw to accept collinear
												// points
		{
			Point[] points = ptt.clone();
			...
			if (size < 0)
				size = 0; // for empty set of points
			return new Polygon(Arrays.copyOf(ans, size));
		}
