...
public class UVA_10652_BOARD_WRAPPING {
	...
	public static void main(String[] args) throws Exception {
		...
		while(n-->0)
		{
			...
			Point [][] points = new Point[boards][4];
			double angles[] = new double [boards];
			double areaBoards = 0.0;
			
			for(int i = 0 ; i < boards ;i++)
			{
				Point [] p = points[i] = new Point [4];
				...
				p[0] = new Point(((new Point(-deltaX, -deltaY)).rotate(angles[i])).x + x, ((new Point(-deltaX, -deltaY)).rotate(angles[i])).y + y) ;
				p[1] = new Point(((new Point(+deltaX, -deltaY)).rotate(angles[i])).x + x, ((new Point(+deltaX, -deltaY)).rotate(angles[i])).y + y);
				p[2] = new Point(((new Point(-deltaX, +deltaY)).rotate(angles[i])).x + x, ((new Point(-deltaX, +deltaY)).rotate(angles[i])).y + y);
				p[3] = new Point(((new Point(+deltaX, +deltaY)).rotate(angles[i])).x + x, ((new Point(+deltaX, +deltaY)).rotate(angles[i])).y + y);
			...
			double areaTotal =(Polygon.convexHull(g)).area();
			
			
			out.printf("%.1f",(areaBoards/areaTotal)*100);
			out.println(" %");
	...
	static Point rotate(double theta, Point p) throws CloneNotSupportedException			//rotate around p
	{
		
		Vector v = new Vector( p,new Point(0, 0));
		Point newP = p.translate(v).rotate(-theta).translate(v.reverse()); 
		return newP;
	...
	static class Point implements Comparable<Point>{
		...
		Point rotate(double theta, Point p) throws CloneNotSupportedException			//rotate around p
		{
			
			Vector v = new Vector( new Point(0,0),p);
			Point newP = translate(v).rotate(-theta); 
			return newP;
		}
