...
public class UVA_1111_TRASH_REMOVAL {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			for (int i = 0; i < n; i++)
				g[i] = new Point(sc.nextDouble(), sc.nextDouble());

			g = Polygon.convexHull(g).g;

			double dist = Double.MAX_VALUE;

			for (int i = 0; i < g.length - 1; i++) {
				double max = 0;
				Line l = new Line(g[i], g[i + 1]);
				for (int j = 0; j < g.length - 1; j++) {
				
					Point cl = l.closestPoint(g[j]);
					max = Math.max(max, cl.dist(g[j]));
				
				}
				
				dist = Math.min(max, dist);
