...
public class UVA_10263_RAILWAY {
	...
	public static void main(String[] args) throws Exception {
		...
		while (sc.ready()) {
			...
			for (int i = 0; i < n; i++) {
				Point q = Point.distToLineSegment(m, p[i], p[i+1]);
				
				double dist1 = q.dist(m);
				if(dist1 < min)
				{
					min = dist1 ; 
					ans = q;
	...
	static class Point implements Comparable<Point> {
		...
		static Point distToLine(Point p, Point a, Point b) // distance between point p and a line defined by points a,
															// b (a != b)
		{
			if (a.compareTo(b) == 0)
				return a;
