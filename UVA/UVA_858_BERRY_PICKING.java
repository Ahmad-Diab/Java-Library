...
public class UVA_858_BERRY_PICKING {
	...
	public static void main(String[] args) throws Exception {
		...
		while (tests-- > 0) {
			...
			for(int i = 0 ; i < n ; i ++ )
			{
				...
				if (Math.abs(p[i].x - q.x) < EPS) {
					a = 1;
					b = 0;
					c = -p[i].x;
				} else {
					a = (p[i].y - q.y) / (q.x - p[i].x);
					b = 1.0;
					c = -(a * p[i].x + p[i].y);
				}
				...
				if(Math.min(p[i].x, q.x) <= x && x <= Math.max(p[i].x, q.x))
			...
			out.println(dist >= threshold ? "YES":"NO");
