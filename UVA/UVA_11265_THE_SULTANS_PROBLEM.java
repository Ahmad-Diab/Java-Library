...
public class UVA_11265_THE_SULTANS_PROBLEM {
	...
	public static void main(String[] args) throws Exception {
		...
		while (sc.ready()) {
			...
			g[2] = new Point(w, h);
			g[3] = new Point(0, h);
			g[4] = g[0] = new Point(0, 0);


			Polygon  curr = new Polygon(g);
			...
			for (int i = 0; i < n; i++) {
				...
				if(Point.ccw(fountain, start, end))
					curr = curr.cutPolygon(start, end);
				else
					curr = curr.cutPolygon(end,start);
	...
	static class Polygon {
		...
		boolean inPolygon(Point p) // O(log n)
		{
			
			int a = 1, b = g.length - 2;
...
//			if(!(size == g.length && (ans[0].x ==g[0].x && ans[size-1].x == g[size-1].x  && (ans[0].y ==g[0].y && ans[size-1].y == g[size-1].y )) ))
//				intersect = true;
//			
